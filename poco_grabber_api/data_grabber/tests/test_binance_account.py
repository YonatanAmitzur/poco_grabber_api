...
from poco_common.core.models import BinanceAccount, make_slug
from poco_common.exchange.serializers import BinanceAccountSerializer

BINANCE_ACCOUNT_URL = reverse('data_grabber:binanceaccount-list')


class PublicBinanceAccountApiTests(TestCase):
    """Test the publicly available BinanceAccount API"""

    def setUp(self):
        self.client = APIClient()

    def test_login_required(self):
        """Test that login is required for retrieving BinanceAccount"""
        res = self.client.get(BINANCE_ACCOUNT_URL)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)


class PrivateBinanceAccountApiTests(TestCase):
    """Test the authorized user BinanceAccount API"""
    ...
    def test_retrieve_binance_account(self):
        """Test retrieving binance account"""

        slug = make_slug()
        curr_user = get_user_model().objects.create_user(
            email='test_another@poco.com',
            password='testpass',
            name='test'
        )

        BinanceAccount.objects.create(
            slug=slug,
            user=curr_user,
            makerCommission=10,
            takerCommission=10,
        ...
        binance_account = BinanceAccount.objects.get(user=self.user.id)
        serializer = BinanceAccountSerializer(binance_account)
        self.assertEqual(res.data[0]['slug'], serializer.data['slug'])
        self.assertEqual(len(res.data), 1)
        self.assertEqual(slug_2, binance_account.slug)
        self.assertEqual(self.user, binance_account.user)

    def test_create_binance_account_successful(self):
        """Test creating a new binance account"""
        ...
        res = self.client.post(BINANCE_ACCOUNT_URL, payload)
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        exists = BinanceAccount.objects.filter(user=self.user.id).exists()
        self.assertTrue(exists)

    def test_create_binance_account_invalid(self):
        """Test creating a new binance account with invalid payload"""
        payload = {
            'makerCommission': 0,
        }
        res = self.client.post(BINANCE_ACCOUNT_URL, payload)
